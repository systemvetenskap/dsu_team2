...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <form runat="server" style="min-height: 20px;margin-bottom: 20px;">
        ...
        <script type='text/javascript'>
            ...
            function GetMessage(cellID, date) {
                ...
                PageMethods.makeBoking(GetCookieID(), cellID, date, null, false, OnGetMessageSuccess, OnGetMessageFailure);
            }
            function BookMany(cellID, date)
            {
                ...
                PageMethods.makeBoking(GetCookieID(), cellID, date, many, true, OnGetMessageSuccess, OnGetMessageFailure);
            }
            ...
            function addFromGolf(result, userContext, methodName)
            {
                ...
                if (!exists) {
                    $('#<%=listBookingmany.ClientID %>').append(result); // adds item 5 at the end
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">Boka tid: <asp:Label ID="lblBookingDetailDateTime" runat="server" Text="Date/Time" class="lblBookingDetailClass" data-provide="typeahead"></asp:Label> </h4>
                    </div>
                    <div class="modal-body">
                        
                        <span>

                        <asp:Label runat="server" id="lblSearch" style="float: left;width: 100%;margin-bottom: 10px;"></asp:Label>
                        <asp:TextBox ID="TextBoxSearch" runat="server" AutoPostBack="false" CssClass="form-control" style="width:200px;margin-top: -5px;margin-left: 0px;float: left;" />
                        <asp:Button ID="addPer" onclientclick="addValToList(); return false;" CssClass="btn btn-default btnsub" data-dismiss="static" Text="Lägg till spelare" runat="server" /></span>
                        <br />
                        <br />
                        <br />
                        <br />
                        <asp:ListBox ID="listBookingmany" runat="server" CssClass="form-control liboxAddMember"></asp:ListBox><br />
                        ...
                        <button id="bookMany" type="button" class="btn btn-default" data-dismiss="modal" >Boka flera medlemmar</button>

                    </div>
                    <div class="modal-footer">
                        
                        <button id="makeBooking" type="button" class="btn btn-default" data-dismiss="modal" >Boka min tid</button>
        ...
        $(document).ready(function () {
            $("#ContentPlaceHolder1_bookingtable td").click(function () {
                if ($(this).hasClass("black")) {
                    ...
                    $("#makeBooking").attr("onclick", "GetMessage('" + colID + "', '" + dat + "'); return false;");
                    $("#bookMany").attr("onclick", "BookMany('" + colID + "', '" + dat + "'); return false;");
                    //substring(0,10)
                    dat = dat.substring(0, 10);
                    var tim = colID.replace("ContentPlaceHolder1_cell", "");
                    tim = tim.substring(0, 2) + ":" + tim.substring(2, 4) + ":00";
                    var writ = dat + "  " + tim;
                    // GetMessage(column_num, tim);
