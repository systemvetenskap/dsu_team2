...
namespace GolfBokning
{
    public partial class myAccount : System.Web.UI.Page
    {
        ...
        private void ButtonMembUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                sql = "UPDATE member SET firstname = @firstname, lastname = @lastname, address = @address, zipcode = @zipcode, place = @place, password = @password WHERE id='" + Convert.ToInt16(myCookie["_id"]) + "' RETURNING id";
            }
            else
            {
                sql = "UPDATE member SET firstname = @firstname, lastname = @lastname, address = @address, zipcode = @zipcode, place = @place WHERE id='" + Convert.ToInt16(myCookie["_id"]) + "' RETURNING id";
            }
